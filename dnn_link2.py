...
class Dnn:
    ...
    def Calc_distort(self, input_dr_link1, input_dr_link2, input_h, R_link1, R_link2, Cx3_link1, Cx3_link2, power, Pout_all_stbc):

        D = np.array([input_dr_link1, input_dr_link2])
        h = np.reshape(input_h,[self.num_link, self.num_link])
        R_link = np.array([R_link1, R_link2])
        Cx3_link = np.array([Cx3_link1, Cx3_link2])
    ...
    def train_dnn(self, input_dr, input_h, num_pl_per_img, lr, Pout_all_stbc, file_path):
        ...
        input_dr_not_scaled_link2 = input_dr_not_scaled[dr_pair:]
        ...
        with tf.device('/CPU:0'):
            ...
            for i in range(len(self.layer_dim_list)):
                ...
                if i < len(self.layer_dim_list)-1:
                    ...
                    mult_link1 = mult[:, : (self.num_R * self.num_pkt) + (self.num_C * self.num_pkt)]
                    mult_link2 = mult[:, - ((self.num_R * self.num_pkt) + (self.num_C * self.num_pkt)) - self.num_link : -self.num_link]
                    ...
                    output_prob_R = tf.stack([output_prob_R_link1, output_prob_R_link2])
                    ...
                    output_prob_C = tf.stack([output_prob_C_link1, output_prob_C_link2])
            ...
            for e in range(self.n_epoch):
                ...
                for j in range(num_batch):
                    ...
                    for h in range(input_h_random.shape[0]):
                        ...
                        R_prob_link1_per_epoch[e,h,j,:] = sess.run(output_prob_R_link1,feed_dict={x_ph: input_batch})
                        R_prob_link2_per_epoch[e, h, j, :] = sess.run(output_prob_R_link2,feed_dict={x_ph: input_batch})

                        C_prob_link1_per_epoch[e,h,j,:] = sess.run(output_prob_C_link1,feed_dict={x_ph: input_batch})
                        C_prob_link2_per_epoch[e, h, j, :] = sess.run(output_prob_C_link2,feed_dict={x_ph: input_batch})
            ...
            dnn_sinr_dB_per_sample = np.zeros([num_pl_per_img, self.num_link, dr_pair])
            dnn_sinr_per_sample = np.zeros([num_pl_per_img, self.num_link, dr_pair])
            dnn_power_per_sample = np.zeros([num_pl_per_img, dr_pair, self.num_link])
            capa_dnn_per_sample = np.zeros([num_pl_per_img, self.num_link, dr_pair])
            ...
            for m in range(dr_pair):
                ...
                for h in range(num_pl_per_img):
                    ...
                    input_concat = np.concatenate((input_link1, input_link2, h_vec),axis=1)

                    dnn_power_per_sample[h] = sess.run(output_pw, feed_dict={x_ph: input_concat})
                    dnn_sinr_dB_per_sample[h] = sess.run(SINR_dB, feed_dict={x_ph: input_concat})
                    dnn_sinr_per_sample[h] = sess.run(SINR, feed_dict={x_ph: input_concat})

                    capa_dnn_per_sample[h, 0, m] = np.log2(1+dnn_sinr_per_sample[h, 0, m])
                    capa_dnn_per_sample[h, 1, m] = np.log2(1 + dnn_sinr_per_sample[h, 1, m])
                    ...
                    distort_dnn_per_sample[h, m, :], tmp_sinr = self.Calc_distort(input_dr_not_scaled_link1[m, :],input_dr_not_scaled_link2[m, :],input_h_not_scaled[m*num_pl_per_img+h, :],
                                                                        dnn_R_link1_per_sample[h, m, :],dnn_R_link2_per_sample[h, m, :],dnn_Cx3_link1_per_sample[h,m,:], dnn_Cx3_link2_per_sample[h,m,:],
    ...
    def test_dnn(self, input_dr, input_h, num_pl_per_img, lr, Pout_all_stbc, file_path):
        ...
        with tf.device('/CPU:0'):
            ...
            dnn_R_prob_link1_per_sample = np.zeros([num_pl_per_img, dr_pair, self.num_pkt*self.num_R])
            dnn_R_prob_link2_per_sample = np.zeros([num_pl_per_img, dr_pair, self.num_pkt*self.num_R])

            dnn_Cx3_link1_per_sample = np.zeros([num_pl_per_img,  dr_pair, self.num_pkt])
            dnn_Cx3_link2_per_sample = np.zeros([num_pl_per_img,  dr_pair, self.num_pkt])

            dnn_Cx3_prob_link1_per_sample = np.zeros([num_pl_per_img, dr_pair, self.num_pkt*self.num_C])
            dnn_Cx3_prob_link2_per_sample = np.zeros([num_pl_per_img, dr_pair, self.num_pkt*self.num_C])

            distort_dnn_per_sample = np.zeros([num_pl_per_img,  dr_pair, self.num_link])
            psnr_dnn_per_sample = np.zeros([num_pl_per_img, dr_pair, self.num_link])

            sinr_per_sample = np.zeros([num_pl_per_img, dr_pair, self.num_link])
            dnn_power_per_sample = np.zeros([num_pl_per_img, dr_pair, self.num_link])
            capa_dnn_per_sample = np.zeros([num_pl_per_img, dr_pair, self.num_link])

            for h in range(num_pl_per_img):
                h_vec = input_h[h::num_pl_per_img, :]
                input_concat = np.concatenate((input_dr_link1, input_dr_link2, h_vec), axis=1)
                ...
                for m in range(dr_pair):
                    distort_dnn_per_sample[h, m, :], sinr_per_sample[h,m,:] = self.Calc_distort(input_dr_not_scaled_link1[m, :],input_dr_not_scaled_link2[m, :],input_h_not_scaled[m*num_pl_per_img+h, :],
                                                                                    dnn_R_link1_per_sample[h, m, :],dnn_R_link2_per_sample[h, m, :],
                                                                                    dnn_Cx3_link1_per_sample[h, m, :],dnn_Cx3_link2_per_sample[h, m, :],dnn_power_per_sample[h,m,:], Pout_all_stbc)

                    capa_dnn_per_sample[h, m, 0] = np.log2(1+sinr_per_sample[h, m, 0])
                    capa_dnn_per_sample[h, m, 1] = np.log2(1 + sinr_per_sample[h, m, 1])
        ...
        for m in range(dr_pair):
            ...
            with open(lr_path + '\Cx3_link1_dnn_test.dat', 'a') as f, open(lr_path + '\Cx3_link2_dnn_test.dat', 'a') as f2:
                for h in range(num_pl_per_img):
                    f.write('%g\t' % (lr))
                    f2.write('%g\t' % (lr))
                    for k in range(self.num_pkt):
                        f.write('%10.10g  ' %dnn_Cx3_link1_per_sample[h, m, k])
                        f2.write('%10.10g  ' % dnn_Cx3_link2_per_sample[h, m, k])
                    f.write('\n')
                    f2.write('\n')
